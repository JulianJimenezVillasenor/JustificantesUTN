        ...
        <ul class="flex justify-between items-center w-full max-w-7xl mx-auto">
            ...
            <li class="flex items-center gap-4">
                <div class="flex flex-col border-l-2 border-emerald-400 pl-4">
                    <span class="text-[10px] uppercase font-bold tracking-[0.2em] text-emerald-300 leading-none">Sistema de
                        Tutorías</span>
            ...
            <li>
                <div
                    class="flex items-center gap-4 px-3 py-1 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                    <div class="hidden md:flex flex-col items-end border-r border-white/20 pr-4">
                        <span
                            class="text-[9px] uppercase font-black tracking-widest text-emerald-300/70 leading-none">Conectado</span>
                        <span class="text-white text-[10px] font-bold opacity-80 italic">Activo</span>
                    </div>

                    @if(Auth::user() && Auth::user()->es_docente)
...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 text-sm">
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <form action="{{ route('tutor.index') }}" method="GET"
                class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex gap-3 items-center">
                ...
                <button type="submit"
                    class="bg-[#004d3d] text-white px-6 py-2.5 rounded-xl font-bold hover:bg-[#00362b] transition-colors shadow-sm">
                    Buscar
                </button>
                @if(request('buscar'))
            ...
            <div class="space-y-4">
                @forelse($justificantes as $j)
                    <div
                        class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative hover:border-[#004d3d]/30 transition-all group">
                        ...
                        <div
                            class="grid grid-cols-2 md:grid-cols-3 gap-4 bg-gray-50/50 p-4 rounded-xl border border-gray-100 mb-6">
                            <div>
                            ...
                            @if($j->status == 'PENDIENTE')
                                <div class="flex gap-2 w-full md:w-auto">
                                    <form action="{{ route('tutor.update', $j->id) }}" method="POST" class="flex-1">
                                        ...
                                        <button type="submit"
                                            class="w-full border-2 border-red-500 text-red-500 px-4 py-2 rounded-xl font-bold hover:bg-red-50 transition-all uppercase text-[11px]">
                                            Rechazar
                                        </button>
                                    </form>
                                    <form action="{{ route('tutor.update', $j->id) }}" method="POST" class="flex-1">
                                        ...
                                        <button type="submit"
                                            class="w-full bg-[#004d3d] text-white px-6 py-2.5 rounded-xl font-bold hover:bg-[#00362b] shadow-md transition-all uppercase text-[11px]">
                ...
                @empty
                    <div class="bg-white p-20 rounded-xl border-2 border-dashed border-gray-200 text-center">
                        ...
                        <a href="{{ route('tutor.index') }}"
                            class="text-[#004d3d] font-bold hover:underline mt-2 inline-block">Ver todos los registros</a>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                <h3 class="font-black text-gray-800 mb-4 border-b pb-2 uppercase text-[11px] tracking-widest">Reportes por
                    Grupo</h3>
                <div class="space-y-3">
                    <select
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-[#004d3d]">
                        <option>DSM-51 (Multiplataforma)</option>
                        <option>DSM-52 (Multiplataforma)</option>
                    </select>
                    <button
                        class="w-full py-3 bg-white border-2 border-[#004d3d] text-[#004d3d] rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-green-50 transition-all">
