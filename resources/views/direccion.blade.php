...
<div class="flex items-center gap-4 px-4 py-1 bg-white/10 rounded-2xl border border-white/5 backdrop-blur-sm">
    <div class="flex flex-col items-end border-r border-white/20 pr-4 hidden sm:flex">
        ...
        <span class="text-white text-xs font-semibold">
            <!--{{ Auth::user('direccion')->name ?? 'Jiménez Villaseñor Julián' }}-->
            {{ Auth::user('direccion')->name ?? 'Dirección' }}
        </span>
    </div>
...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <a href="{{ route('direccion.viewReports') }}" class="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded">Ver Reportes</a>
    </div>
</div>
@endsection
